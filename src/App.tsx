...
export function App(){const[config,setConfig]=useState<Config>(DEFAULT_CONFIG),[borders,setBorders]=useState<Border[]>([]),[rice,setRice]=useState<Rice[]>([]),[slides,setSlides]=useState<Slider[]>([]),[loading,setLoading]=useState(true),[syncing,setSyncing]=useState(false),[admin,setAdmin]=useState(!!getToken()),[loginOpen,setLoginOpen]=useState(false),[settingsOpen,setSettingsOpen]=useState(false),[edit,setEdit]=useState<Border|null>(null),[addOpen,setAddOpen]=useState(false),[toast,setToast]=useState(''),[slide,setSlide]=useState(0),[search,setSearch]=useState('');const pdfRef=useRef<HTMLDivElement>(null);const [form,setForm]=useState<any>({name:'',mealCount:0,mealRate:0,extraCost:0,miscCost:0,totalDeposit:0});
 ...
 const hydrate=async()=>{try{setSyncing(true);const r=await load(config);setConfig(c=>({...c,...r.data.config}));setBorders(r.data.borders);setRice(r.data.rice);setSlides(r.data.sliderImages);notify('Google Sheets থেকে সব তথ্য এক API কলে আপডেট হয়েছে।')}catch(e:any){notify(e.message||'সিঙ্ক ব্যর্থ হয়েছে।')}finally{setLoading(false);setSyncing(false)}};
 useEffect(()=>{if(config.appScriptUrl)hydrate();else setLoading(false)},[]);useEffect(()=>{if(slides.length<2)return;const t=setInterval(()=>setSlide(s=>(s+1)%slides.length),3000);return()=>clearInterval(t)},[slides.length]);
 ...
 const persist=async(nb=borders,nr=rice,ns=slides,nc=config)=>{try{await saveAll(nc,nb,nr,ns);notify('Google Sheets সফলভাবে আপডেট হয়েছে।')}catch(e:any){notify(e.message||'সংরক্ষণ ব্যর্থ হয়েছে।')}};
